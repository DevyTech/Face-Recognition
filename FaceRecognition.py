...
def markFace(name):
    with open('RecordFace.csv','r+') as f:
...
while True:
    ...
    encodesCurFace = face_recognition.face_encodings(img, facesCurFrame)
    ...
    for encodeFace,faceLoc in zip(encodesCurFace,facesCurFrame):
        ...
        if matches[matchIndex]:
            ...
            cv2.puText(img,name,(x1+6,y2-6),cv2.FONT_HERSHEY_COMPLEX,1,(255,255,255),2)
            markFace(name)
